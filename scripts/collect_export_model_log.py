...
from utils.result import Measurement, Result, xstr

parser = argparse.ArgumentParser(allow_abbrev=False)
parser.add_argument("--framework", type=str, required=True)
parser.add_argument("--frameworks", type=str, required=True)
...
parser.add_argument("--data_source", type=str, default="Source")
parser.add_argument("--python_version", type=str, default="unknown")
...
nightly_cpu_list = [
    "clx8280-070", "clx8280-071", "clx8280-072", "clx8280-073", "clx8260-136", "clx8260-137",
    "clx8280-0769"
]
...
class ExportModelResult(Result):
            ...
            for benchmark in self.benchmarks:
                line = ";".join([
                    xstr(item) for item in [
                        ...
                        args.frameworks,
                        self.version,
                        benchmark.precision.upper(),  # int8 || fp32
                        self.model,  # model name
                        self.data_source,  # Source or Target
                        benchmark.simple_mode,  # performance or accuracy
                        benchmark.batch_size,
                        benchmark.value,
                        f"{self.model}/{benchmark.log}",
                    ]
                ])
                data.append(line)
            return data
...
def get_config_tree(log_files):
    ...
    for log_file in log_files:
        if "export.log" in log_file:
            continue
        params = os.path.basename(log_file).replace(f"{args.framework}-{args.model}-",
                                                    "").split("-")
        precision = params[0]
        mode = params[1]
...
def read_perf_logs(precision, mode):
    logs_pattern = os.path.join(
        args.logs_dir,
        f"{args.framework}-{args.model}-{precision}-{mode}*",
    )
...
def parse_perf_line(mode: str, line: str) -> dict:
    ...
    accuracy_patterns = [r"Accuracy:\s+(\d+(\.\d+)?)", r"Accuracy is\s+(\d+(\.\d+)?)"]
...
def main():
    ...
    tuning_log = os.path.join(args.logs_dir, f"{args.framework}-{args.model}-int8-export.log")
    if os.path.exists(tuning_log):
        read_tuning_log(tuning_log)

    logs_pattern = os.path.join(args.logs_dir, f"{args.framework}-{args.model}-int8-export.log")
    log_files = glob.glob(logs_pattern)
