...
main() {
    ...
    if [ "${framework}" == "onnxrt" ]; then
        ...
        if [[ ${model} == "faster_rcnn" ]] || [[ ${model} == "mask_rcnn" ]] || [[ ${model} == "yolov3" ]] || [[ ${model} == "yolov4" ]] || [[ ${model} == "tiny_yolov3" ]] || [[ ${model} == "ultraface" ]] || [[ ${model} == "emotion_ferplus" ]] || [[ ${model} == "arcface" ]] || [[ " ${qdq_model_list[@]} " =~ " ${model} " ]];then
