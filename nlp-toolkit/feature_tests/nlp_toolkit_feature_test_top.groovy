            ...
            job_features.each { job_feature ->
                ...
                jobs["${job_feature}"] = {
                    ...
                    catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                        downstreamJob = build job: "nlp-toolkit-feature-test", propagate: false, parameters: featureParams

                        catchError {
                            copyArtifacts(
                                    projectName: "nlp-toolkit-feature-test",
...
def generateReport(){
    dir(WORKSPACE) {
        ...
        withEnv([
                "lpot_branch=${nlp_branch}",
                "lpot_commit=${nlp_commit}",
                ...
                job_features.each { feature ->
                    ...
                    dir(WORKSPACE) {
                        withEnv([
                            ...
                            sh '''
                        chmod 775 ./lpot-validation/nlp-toolkit/feature_tests/collect_log/collect_log_${feature_name}.sh
                        ./lpot-validation/nlp-toolkit/feature_tests/collect_log/collect_log_${feature_name}.sh
                    '''
                        }
                    }
                }
