...
function main {
    ...
    cd ${WORKSPACE}/lpot-models/examples/optimization/pytorch/huggingface/text-classification/distillation
    n=0
    until [ "$n" -ge 5 ]
    do
    ...
    python run_glue.py --model_name_or_path Intel/distilbert-base-uncased-sparse-90-unstructured-pruneofa \
        ...
        --task_name sst2 --distillation --do_eval --do_train \
        --per_device_eval_batch_size 16 --output_dir ./saved_distillation \
        --overwrite_cache --max_train_samples 100 --max_eval_samples 50 --max_predict_samples 50 \
        --overwrite_output_dir 2>&1 | tee ${WORKSPACE}/pytorch_distillation.log
