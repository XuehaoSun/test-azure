...
function main {
    ...
    cd ${WORKSPACE}/lpot-models/examples/optimization/pytorch/huggingface/text-classification/distillation_for_quantization
    n=0
    until [ "$n" -ge 5 ]
    do
    ...
    pip list
    python run_glue.py --task_name qqp --model_name_or_path yoshitomo-matsubara/bert-base-uncased-qqp --per_device_train_batch_size 64 --per_device_eval_batch_size 64 --max_train_samples 1024 --max_eval_samples 1024 --do_train --do_eval --pad_to_max_length --num_train_epochs 5 --output_dir saved_results 2>&1 | tee ${WORKSPACE}/pytorch_pruning_qqp.log
