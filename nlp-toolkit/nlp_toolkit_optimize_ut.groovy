            ...
            stage('unit test') {
                ...
                timeout(80) {
                    ...
                    python ${WORKSPACE}/lpot-validation/scripts/get_coverage_summary.py \
                        ...
                        --summary-file=${WORKSPACE}/coverage_summary_optimize.log
                    '''
                    lines_coverage = Float.parseFloat(sh(
                            script: "grep 'lines_coverage' ${WORKSPACE}/coverage_summary_optimize.log | cut -d ',' -f 4",
                            returnStdout: true
                    ).trim())
                    println("Lines coverage: " + lines_coverage)

                    branches_coverage = Float.parseFloat(sh(
                            script: "grep 'branches_coverage' ${WORKSPACE}/coverage_summary_optimize.log | cut -d ',' -f 4",
                            returnStdout: true
                    ).trim())
                    ...
                    if (MR_source_branch == "") {
                        ...
                        withCredentials([string(credentialsId: '2f98cfad-c470-4c49-a85a-43c236507236', variable: 'SIGOPT_TOKEN')]) {
                            ...
                            python ${WORKSPACE}/lpot-validation/scripts/get_coverage_summary.py \
                                ...
                                --summary-file=${WORKSPACE}/coverage_summary_optimize_base.log
                            '''
                        }
                        lines_coverage_base = Float.parseFloat(sh(
                                script: "grep 'lines_coverage' ${WORKSPACE}/coverage_summary_optimize_base.log | cut -d ',' -f 4",
                                returnStdout: true
                        ).trim())
                        branches_coverage_base = Float.parseFloat(sh(
                                script: "grep 'branches_coverage' ${WORKSPACE}/coverage_summary_optimize_base.log | cut -d ',' -f 4",
                                returnStdout: true
                        ).trim())
                        try {
