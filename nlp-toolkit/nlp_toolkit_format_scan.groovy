...
node(node_label) {
    try {
        ...
        stage("Code Scan") {
            ...
            echo "---------------------------------------------------------"
            status = sh(
            script: "chmod +x ${WORKSPACE}/nlp-toolkit/scripts/run_nlp_toolkit_format_scan.sh && ${WORKSPACE}/nlp-toolkit/scripts/run_nlp_toolkit_format_scan.sh --python_version=${python_version} --tool=${TOOL} --repo_dir=${WORKSPACE}/nlp_toolkit",  // There is no source branch as script assumes that it is currently on MR branch; look at download function.
            returnStatus:true)
            if (status != 0) {
