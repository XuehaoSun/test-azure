...
def get_jira_data(project, version, type_list = []):
    ...
    for type in type_list:
        ...
        jql = "project%20%3D%20{2}%20AND%20issuetype%20%3D{3}%20AND%20Sprint%20%3D%22{0}%20v{1}%22".format(
            project, version, jira_project, type)
        cmd = "curl -o out_{2}.json -x 'http://child-prc.intel.com:913' -k -vv -H \"Authorization: Bearer NDYwOTMxMzA3Nzk2OoFBUadiAkGCyKaiq3QBVq28F+Iw\" -H \"content-type: application/json\" -X GET {0}?jql={1}".format(
            jira_api_url, jql, type)
        os.system(cmd)
...
def search(json_name):
    ...
    for issue in jira_data["issues"]:
        ...
        if (issue_dict['Jira Status'] == "Closed"):
