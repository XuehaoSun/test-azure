...
def codeScan(tool) {
    ...
    downstreamJob = build job: "intel-neural-coder-format-scan", propagate: false, parameters: codeScanParams

    copyArtifacts(
        projectName: "intel-neural-coder-format-scan",
        selector: specific("${downstreamJob.getNumber()}"),
        filter: '*.json,*.log,*.csv',
        fingerprintArtifacts: true,
        target: "format_scan",
    ...
    if (tool != "cloc") {
        ...
        writeFile file: "${overview_log}", text: text_comment + "intel-neural-coder-format-scan," + tool + "," + downstreamJob.result + "," + downstreamJob.number + "\n"
    }
