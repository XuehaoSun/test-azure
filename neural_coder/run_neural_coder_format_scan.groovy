        ...
        withEnv(["python_version=${python_version}", "CPU_NAME=${CPU_NAME}"]) {
            retry(5) {
                sh '''#!/bin/bash
                    ...
                    conda_env_name=neural-coder-format_scan-${python_version}
                    if [[ -n ${CPU_NAME} ]]; then
...
node(node_label) {
    try {
        ...
        stage("Code Scan") {
            ...
            echo "---------------------------------------------------------"
            status = sh(
            script: "chmod +x ${WORKSPACE}/neural_coder/run_neural_coder_format_scan.sh && ${WORKSPACE}/neural_coder/run_neural_coder_format_scan.sh --python_version=${python_version} --tool=${TOOL} --repo_dir=${WORKSPACE}/lpot",  // There is no source branch as script assumes that it is currently on MR branch; look at download function.
            returnStatus:true)
            if (status != 0) {
