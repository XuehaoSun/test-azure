...
class eval_classifier_optimized_graph:
    ...
    def __init__(self):
        ...
        arg_parser.add_argument('-a', "--num-intra-threads",
                                ...
                                dest='num_intra_threads', type=int, default=28)
        ...
        arg_parser.add_argument("--warmup-steps", type=int, default=5,
                                help="number of warmup steps")
        arg_parser.add_argument("--steps", type=int, default=50,
                                help="number of steps")
        arg_parser.add_argument("--config", default='./tf.yaml',
                                help="tuning config")
    ...
    def auto_tune(self):
        ...
        if self.args.tune:
            ...
            if not q_model:
                print("Not find q_model.")
                return
            bs = 100
            tracemalloc.start()
            top1, batch_time = self.inference(q_model)
            _, peak = tracemalloc.get_traced_memory()
            tracemalloc.stop()
            print("Tuning peek is: %.1f" % peak)

            print("q_model accuracy batch_size: %d" % bs)
            print("q_model accuracy: %f " % top1)
    ...
    def inference(self, infer_graph):
        ...
        with data_graph.as_default():
            if (self.args.data_location):
                ...
                preprocessor = dataset.get_image_preprocessor()(
                    self.args.image_size, self.args.image_size, bs,
                ...
                input_shape = [bs, self.args.image_size, self.args.image_size, 3]
        ...
        while num_remaining_images >= bs and iteration < total_run:
            iteration += 1
            ...
            if self.args.label_adjust:
                np_labels -= 1
            num_processed_images += bs
            num_remaining_images -= bs
