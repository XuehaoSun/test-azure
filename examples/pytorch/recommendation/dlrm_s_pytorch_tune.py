...
if __name__ == "__main__":
    ...
    def inference(model):
        ...
        targets = np.concatenate(targets, axis=0)
        acc = sklearn.metrics.roc_auc_score(targets, scores)
        throughput = iter * args.test_mini_batch_size / total_time
        print("acc:", acc)
    ...
    if args.tune:
        ...
        acc, speed = inference(q_model)
        print("q_model accuracy batch_size: %d" % args.test_mini_batch_size)
        print("q_model accuracy: %f" % acc)
        print("q_model throughput batch_size: %d" % args.test_mini_batch_size)
    ...
    if args.fp32_benchmark:
        ...
        acc, speed = inference(dlrm)
        print("input_model accuracy batch_size: %d" % args.test_mini_batch_size)
        print("input_model accuracy: %f" % acc)
        print("input_model throughput batch_size: %d" % args.test_mini_batch_size)
