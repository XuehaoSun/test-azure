...
'''
model_list = ['sgan']
# 'acgan', 'sgan', 'began' , 'cgan', 'wgan', 'ebgan', 'dcgan', 'srgan'
...
for model in model_list:
    ...
    # commond = "python {}/{}.py --precision lpot --batch_size 1 --inference --num-iterations 100".format(model, model)
    # if model == 'srgan':
    #     commond += ' --latent_dim 16'
    ...
    # commond = "python ./main.py -e --data /lustre/dataset/imagenet/img_raw/ --performance --pretrained \
    #             --no-cuda -j 1 -a {} -b 1 --precision int8".format(model)
    # commond = "python acgan.py --precision int8 --batch_size 1 --inference --num-iterations 100"
    # subprocess.check_output(commond, shell=True)
    ...
    commond = commond = "python {}/{}.py --batch_size 1 --inference --num-iterations 100".format(model, model)
    if model == 'srgan':
        commond += ' --latent_dim 16'
    for n in range(process_num):
        head = "numactl --localalloc --physcpubind {},{},{},{}  timeout 3600 ".format(i, i+1, i+2, i+3)
