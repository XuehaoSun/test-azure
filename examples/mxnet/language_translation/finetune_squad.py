...
tune = args.tune
fp32_benchmark = args.fp32_benchmark
...
def evaluate(net):
    ...
    return F1_EM["exact_match"], total_num / (epoch_toc - epoch_tic)
...
if __name__ == '__main__':
    if only_calibration:
        ...
        if tune:
            ...
            q_model = bert_tuner.tune(net, q_dataloader=dev_dataloader, eval_dataloader=dev_dataloader, eval_func=eval_func)
            end_tune = time.time()
            print("Tuning time spend: %.1f s" % (end_tune-start_tune))
            tracemalloc.start()
            top1, speed = evaluate(q_model)
