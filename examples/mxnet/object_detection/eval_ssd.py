...
def get_dataset(dataset, data_shape):
    if dataset.lower() == 'voc':
        ...
        val_dataset = gdata.COCODetection(splits='instances_val2017', skip_empty=False, root=args.data_location)
...
if __name__ == '__main__':
    ...
    def test_func(graph):
        ...
        ctx = [mx.cpu()]
        results, speed = validate(graph, val_data, ctx, classes, size, val_metric)

        mAP = float(results[-1][-1])

        return mAP

    def inference(graph, bs):
        val_dataset, val_metric = get_dataset(args.dataset, args.data_shape)
        val_data = get_dataloader(
            val_dataset, args.data_shape, bs, args.num_workers)
        classes = val_dataset.classes  # class names
    ...
    if args.tune:
        ...
        q_model = bert_tuner.tune(net, q_dataloader=val_data, eval_dataloader=val_dataset, eval_func=test_func)
        end_tune = time.time()
