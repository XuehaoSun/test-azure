...
class OneDNNLog:
    ...
    def dump_verbose_static(self):
        res = {}
        dataf = self.df
        dataf_len = len(dataf.index)
        dataf_title = {}
        for i in range(len(dataf.columns)):
            dataf_title.update({dataf.columns[i]: i})

        for i in range(dataf_len):
            each_line = dataf.loc[i]
            primitive = each_line[dataf_title['primitive']]
            impl = each_line[dataf_title['impl']]
            prop = each_line[dataf_title['prop']]
            iotypes = each_line[dataf_title['iotypes']]
            postops = each_line[dataf_title['postops']]
            prob = each_line[dataf_title['prob']]
            elapsed = each_line[dataf_title['elapsed']]
            if str(primitive) in ['convolution', 'gemm', 'matmul', 'inner_product']:
                op_desc = str(primitive) + '-' + str(prob)
                op_time = elapsed

                if op_desc not in res.keys():
                    res.update({op_desc: [1, op_time]})
                else:
                    total_call_num = res[op_desc][0] + 1
                    op_avg_time = ( res[op_desc][1] + op_time ) / total_call_num
                    res.update({op_desc: [total_call_num, op_avg_time]})
...
def process():
    ...
    parser.add_argument('--dump_d',
                        action='store_true',
                        default=False,
                        help='dump detail OneDNN op with diff shape')
    ...
    if args.dump_d:
        # print(dlog.get_df().loc[:, COLS].to_string())
        verbose_op_detail = dlog.dump_verbose_static()
        for key, val in verbose_op_detail.items():
